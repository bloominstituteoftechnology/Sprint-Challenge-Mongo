  ...
  return (req, res, next) => {
    if (!req.params.id) res.status(400).json({ errorMessage: "Please specify an ID in the url" });
      ...
      goose
        ...
        .catch(error => res.status(500).json({ error: 'Error deleting document' }));
    }
  }
}
...
const sanitizeMiddleware = (type) => {
  return (req, res, next) => {
    ...
    if (type === "expense") {
      ...
      if (expense.amount === undefined || expense.description === undefined || expense.budget === undefined || expense.category === undefined) {
